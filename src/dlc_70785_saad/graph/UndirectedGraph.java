...
public class UndirectedGraph<T> extends Graph<T> {
    ...
    public List<Node<T>> getNeighboursForNode(Node<T> node) {
        ...
        if (vertices.contains(node)) {
            for (Arc<T> arc : node.getArcs()) {
                ...
                } else if (arc.getEnd().equals(node)) {
                    listOfNodes.add(arc.getInit());
                }
            }
        }
    ...
    public GraphDepthFirstParser<T> getGraphDepthFirstParser() {
        return new UndirectedGraphDepthParser();
    }

    private class UndirectedGraphDepthParser extends GraphDepthFirstParser<T> {

        private String path;
        private int components;

        public UndirectedGraphDepthParser(){
            super();
        }
