...
public class DirectedGraph<T> extends Graph<T> {
    ...
    /**
     * Crea un grafo dirigido, con lista de vértices vacía, lista de arcos vacía
     * y sin permitir arcos paralelos.
     */
    ...
    /**
     * Crea un grafo dirigido cuya lista de vértices será <b>v</b> y cuya lista
     * de arcos será <b>a</b>, sin permitir arcos paralelos. El método no
    ...
    /**
     * Crea un grafo dirigido cuya lista de vértices será <b>v</b> y cuya lista
     * de arcos será <b>a</b>. El parámetro p indica si el grafo aceptará arcos
     * paralelos (p = true) o no (p = false). El método no controla si las
    ...
    /**
     * Crea un arco dirigido con in como vértice de partida y en como vértice de
     * llegada. El peso del arco será w. No comprueba si las referencias in y en
     * son null.
     *
    ...
    public List<Node<T>> getReachablesNodesForNode(Node<T> node) {
        ...
        if (vertices.contains(node)) {
            for (Arc<T> arc : node.getArcs()) {
                if (arc.getInit().equals(node)) {
                    listOfNodes.add(arc.getEnd());
                }
            }
        }
    ...
    @Override
    public GraphDepthFirstParser<T> getGraphDepthFirstParser() {
        return new DirectedGraphDepthParser();
    }

    private class DirectedGraphDepthParser extends GraphDepthFirstParser<T> {

        private String path;

        public DirectedGraphDepthParser(){
            super();
        }

        @Override
        protected void performDeepFirstTravel() {
            ...
            for (Node<T> node : vertices) {
                if (!visited.contains(node)){
                    path.append(visitInDepth(node, visited));
                }
            }
        ...
        public int countComponents() {
            throw new UnsupportedOperationException("Only for undirected graphs");
        }
    }
